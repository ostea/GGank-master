...
package com.gankmz.library.diskcache.lru;

import android.util.Log;

import com.gankmz.library.diskcache.CacheEntry;
import com.gankmz.library.diskcache.DiskCache;
import com.gankmz.library.diskcache.FileUtils;
import com.gankmz.library.diskcache.SimpleHashSet;
...
public final class LruActionTracer implements Runnable {
        ...
        if (mJournalFile.exists()) {
            try {
                ...
                if (SimpleDiskLruCache.DEBUG) {
                    Log.d(SimpleDiskLruCache.LOG_TAG, "open success");
                }
            } catch (IOException journalIsCorrupt) {
                journalIsCorrupt.printStackTrace();
                if (SimpleDiskLruCache.DEBUG) {
                    Log.d(SimpleDiskLruCache.LOG_TAG, "clear old cache");
                }
                clear();
            }
        } else {
            ...
            if (SimpleDiskLruCache.DEBUG) {
                Log.d(SimpleDiskLruCache.LOG_TAG, "create new cache");
            }
    ...
    public synchronized void clear() throws IOException {
        ...
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "delete directory");
        }
    ...
    public synchronized CacheEntry beginEdit(String key) throws IOException {
        ...
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "beginEdit:"+key);
        }
        CacheEntry cacheEntry = mLruEntries.get(key);
        if (cacheEntry == null) {
    ...
    public void abortEdit(CacheEntry cacheEntry) {
        ...
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "abortEdit: %s"+cacheKey);
        }
    ...
    public void commitEdit(CacheEntry cacheEntry) throws IOException {
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "commitEdit: %s"+cacheEntry.getKey());
        }
    ...
    private void waitJobDone() {
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "waitJobDone");
        }
        ...
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "job is done");
        }
    }
    ...
    private synchronized void trimToSize() {
        ...
        if (mSize > mCapacity) {
            if (SimpleDiskLruCache.DEBUG) {
                Log.d(SimpleDiskLruCache.LOG_TAG, "should trim, current is: %s"+mSize);
            }
        }
    ...
    public synchronized boolean delete(String key) throws IOException {
        if (SimpleDiskLruCache.DEBUG) {
            Log.d(SimpleDiskLruCache.LOG_TAG, "delete: %s"+ key);
        }
        checkNotClosed();
        validateKey(key);
        CacheEntry cacheEntry = mLruEntries.get(key);
