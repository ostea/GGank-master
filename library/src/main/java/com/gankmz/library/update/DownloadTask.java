...
import com.gankmz.library.cache.DiskFileUtils;
import com.gankmz.library.concurrent.SimpleTask;
...
public class DownloadTask extends SimpleTask {
    ...
    public void doInBackground() {
        ...
        if (isCancelled()) {
            Log.d(LOG_TAG, "task has been canceled");
            return;
        }

        if (totalRead != totalSize) {
            Log.d(LOG_TAG, "download fail, file not complete");
            setResult(RESULT_DOWNLOAD_ERROR);
        } else {
